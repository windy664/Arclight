...
public abstract class CommandsMixin implements CommandsBridge {
    ...
    @Shadow @Final private static ClientboundCommandsPacket.NodeInspector<SharedSuggestionProvider> COMMAND_NODE_INSPECTOR;
    @Shadow
    ...
    public void sendCommands(ServerPlayer player) {
        ...
        bridge$forge$mergeNode(vanillaCommands.getDispatcher().getRoot(), vanillaRoot, map, source, ctx -> 0, provider -> (SuggestionProvider<SharedSuggestionProvider>) (Object) provider);

        RootCommandNode<SharedSuggestionProvider> node = new RootCommandNode<>();
        map.put(this.dispatcher.getRoot(), node);
        bridge$forge$mergeNode(this.dispatcher.getRoot(), node, map, source, ctx -> 0, provider -> (SuggestionProvider<SharedSuggestionProvider>) (Object) provider);
